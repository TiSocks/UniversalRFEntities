"""Binary Sensor platform for RF Entities."""
...
async def async_setup_entry(
    ...
    """Set up the RF Binary Sensor entities from a config entry."""
    ...
    for slug, sens_config in sensors_config.items():
        if sens_config.get(CONF_SENSOR_TYPE) == "binary":
            entities.append(
                RFBinarySensorEntity(
                    hass=hass,
                    entry_id=entry_id,
                    ...
                    auto_off_sec=sens_config.get(CONF_SENSOR_AUTO_OFF_SEC, 0),
...
class RFBinarySensorEntity(BinarySensorEntity):
    """Representation of an RF binary sensor."""
    ...
    def __init__(
        ...
        """Initialize the RF Binary Sensor."""
        self.hass = hass
        self._entry_id = entry_id
        self._device_name = device_name
        self._slug = slug
        self._attr_name = name
        self._attr_unique_id = f"{entry_id}_binary_{slug}"
        self._state_on_code = state_on_code
        self._state_off_code = state_off_code
        self._auto_off_sec = auto_off_sec
    ...
    def _handle_rf_event(self, event) -> None:
        ...
        # Check if code matches configured state triggers
        if self._state_on_code and code == self._state_on_code:
            self._is_on = True
            self._update_event_attributes(data)
            self.async_write_ha_state()

            # Manage auto-off timer
            if self._cancel_auto_off:
                self._cancel_auto_off()
                self._cancel_auto_off = None

            if self._auto_off_sec > 0:
                @callback
                def _handle_auto_off(now):
                    self._is_on = False
                    self._cancel_auto_off = None
                    self.async_write_ha_state()

                self._cancel_auto_off = async_call_later(
                    self.hass, self._auto_off_sec, _handle_auto_off
                )
    ...
    def _update_event_attributes(self, data: dict[str, Any]) -> None:
        """Update sensor attributes with event metadata."""
        self._extra_attributes["last_code"] = data.get("code") or data.get("value")
